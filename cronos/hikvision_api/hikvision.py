...
def fetch_data():
    settings = frappe.get_doc("CRONOS HikVision Settings", "CRONOS HikVision Settings")
...
def fetch_hik_vision_records():
    ...
    for record in records:
        if not frappe.db.exists("Cronos Hik Vision Attendance", {"id": record['ID_Global']}):
            doc = frappe.get_doc({
                "doctype": "Cronos Hik Vision Attendance",
                "id": record['ID_Global'],
...
@frappe.whitelist()
def delete_records():
    frappe.db.delete("Cronos Hik Vision Attendance")
